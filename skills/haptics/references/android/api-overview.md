...
See `pattern-composer.md` in this directory for the full PatternComposer API: `PatternData`, `ContinuousPattern`, `ConfigPoint`, `ValuePoint` types, methods, example, and Android version-specific notes (Envelope API / API 26 behavior).
...
See `types-reference.md` in this directory for the full Kotlin type definitions: `PatternData`, `ContinuousPattern`, `ValuePoint`, `ConfigPoint`, `Preset`, `CompatibilityMode`, and `RealtimeComposerStrategy`.
