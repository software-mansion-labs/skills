...
See `pattern-composer.md` in this directory for the full PatternComposer API, types (`PatternData`, `ContinuousPattern`, `ValuePoint`, `DiscretePoint`), methods, and SwiftUI usage examples. The section below is a short summary — consult `pattern-composer.md` before building a custom pattern.
