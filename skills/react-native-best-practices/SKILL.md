...
| `references/on-device-ai/SKILL.md` | On-device AI: LLMs (chat, tool calling, structured output), computer vision (classification, object detection, OCR, segmentation, style transfer, embeddings, text-to-image), speech processing (STT, TTS, VAD), VisionCamera real-time frame processing, model loading, resource management, custom models with ExecuTorch |
