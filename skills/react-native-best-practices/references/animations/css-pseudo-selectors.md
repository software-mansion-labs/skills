...
- `default` is the resting value. Leaving it out means the selector animates back to the property's own default value (transparent for a color, `1` for `opacity`), by design; the pseudo object owns the property, so a value from an earlier entry of the style array is not used. To rest at a `StyleSheet` value, write it as `default`.
