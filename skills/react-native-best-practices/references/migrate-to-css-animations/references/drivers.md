...
A discrete UI-thread event: a worklet gesture callback that fires once per interaction (`onStart`, `onEnd`, `onFinalize` without `.runOnJS(true)`; the callbacks run on the UI thread only when every callback is a worklet, inline or marked `'worklet'`), or a one-off `runOnUI`/`scheduleOnUI` body. `runOnUI` is how a write reaches the UI runtime, not a source: propose moving the write to the JS thread (`.runOnJS(true)` on the gesture, `scheduleOnRN` inside the worklet, or set state where `runOnUI` was called) and holding the target in state, as Needs approval, saying that the animation then starts after a JS round trip and a render instead of on the next UI frame. A body that also does UI-thread work of its own (`scrollTo`, a gesture state) stays on shared values. A press-shaped begin/end pair is question 8.
